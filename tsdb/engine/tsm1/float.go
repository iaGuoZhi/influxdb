...
func (s *FloatEncoder) Write(v float64) {
	...
	if vDelta == 0 {
		...
		if trailing < 6 {
			...
			s.bw.WriteBits(previousIndex * 32 + 16 + leadingRepresentation, previousValuesLog2 + 5)
			...
			//s.bw.WriteBit(bitstream.One)
			//s.bw.WriteBits(previousIndex, previousValuesLog2)
			s.bw.WriteBits(previousIndex * 32 + 16 + 8 + leadingRepresentation, previousValuesLog2 + 5)
